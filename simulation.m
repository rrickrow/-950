...
%   2. Simulates the drive-response CVINNN system with actuator saturation
...
T_END = 200;          % simulation end time (s) – extended to show synchronisation
t_arr = 0 : dt : T_END;
N_STEP = length(t_arr);
...
fprintf('\nStarting simulation with actuator saturation (u_max = %.1f) ...\n', U_MAX_SAT);
...
fprintf('\n══ Simulation Summary ══════════════════════════════════\n');
fprintf('  Max |U| with saturation (u_max=%.1f):   %.4f\n', U_MAX_SAT, max(abs(U_ctrl(:))));
fprintf('  Error norm at t= 80 s:                   %.6f\n', err_norm_sat(round( 80/dt)+1));
fprintf('  Error norm at t=150 s:                   %.6f\n', err_norm_sat(round(150/dt)+1));
fprintf('  Error norm at t=%g s:                   %.6f\n',  T_END, err_norm_sat(end));
fprintf('════════════════════════════════════════════════════════\n\n');
...
for k = 1:4
    subplot(2,2,k);
    plot(t_arr(1:idx40), extract{k}(m1(1:idx40,:)), 'b',   'DisplayName','Drive');
    hold on;
    plot(t_arr(1:idx40), extract{k}(m2(1:idx40,:)), 'r--', 'DisplayName','Response');
    xlabel('Time [s]'); ylabel(comp_lbl{k}); grid on;
    legend('Location','best','FontSize',8);
end
sgtitle('Trajectories of states (first 40 s, before synchronisation)','FontWeight','bold');

% ── Figure 2: State trajectories over full simulation ────────────────────────
figure('Name','States with actuator saturation','Position',[100 50 900 600]);
for k = 1:4
    subplot(2,2,k);
    plot(t_arr, extract{k}(m1), 'b',   'DisplayName','Drive');
    hold on;
    plot(t_arr, extract{k}(m2), 'r--', 'DisplayName','Response');
    xlabel('Time [s]'); ylabel(comp_lbl{k}); grid on;
    legend('Location','best','FontSize',8);
end
...
% ── Figure 3: Error states ────────────────────────────────────────────────────
figure('Name','Error states','Position',[150 50 900 600]);
...
sgtitle(sprintf('Synchronisation error with actuator saturation (u_{max}=%.1f)', ...
                U_MAX_SAT), 'FontWeight','bold');

% ── Figure 4: Control inputs ──────────────────────────────────────────────────
figure('Name','Control inputs (saturated)','Position',[200 50 900 600]);
u_lbl = {'\Re(U_1)', '\Im(U_1)', '\Re(U_2)', '\Im(U_2)'};
u_ex  = {@(u)real(u(:,1)), @(u)imag(u(:,1)), @(u)real(u(:,2)), @(u)imag(u(:,2))};
for k = 1:4
    subplot(2,2,k);
    plot(t_arr, u_ex{k}(U_ctrl), 'r', 'DisplayName', ...
...
sgtitle(sprintf('Control inputs with actuator saturation (u_{max}=%.1f)', ...
                U_MAX_SAT), 'FontWeight','bold');

% ── Figure 5: Error norm (log scale) – extended t-axis to show synchronisation
...
title(sprintf('Synchronisation error norm (u_{max}=%.1f, T_{end}=%g s)', ...
              U_MAX_SAT, T_END), 'FontWeight','bold');
...
for k = 1:4
    subplot(2,2,k);
    plot(t_arr(1:idx20), u_ex{k}(U_ctrl(1:idx20,:)), 'r', 'DisplayName','Saturated');
    hold on;
